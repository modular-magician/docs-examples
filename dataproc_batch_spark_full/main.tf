...
resource "google_dataproc_batch" "example_batch_spark" {
    batch_id      = "dataproc-batch-${local.name_suffix}"
...
resource "google_storage_bucket" "bucket" {
  ...
  name                        = "dataproc-bucket-${local.name_suffix}"
  location                    = "US"
  force_destroy               = true
}
...
resource "google_dataproc_cluster" "basic" {
  name   = "dataproc-batch-${local.name_suffix}"
  region = "us-central1"
 ...
 resource "google_dataproc_metastore_service" "ms" {
  service_id = "dataproc-batch-${local.name_suffix}"
  location   = "us-central1"
  port       = 9080
  tier       = "DEVELOPER"
