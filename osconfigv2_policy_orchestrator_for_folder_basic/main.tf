resource "google_folder" "my_folder" {
    display_name        = "po-folder-${local.name_suffix}"
...
resource "google_os_config_v2_policy_orchestrator_for_folder" "policy_orchestrator_for_folder" {
    ...
    policy_orchestrator_id = "po-folder-${local.name_suffix}"
    ...
    orchestrated_resource {
        id = "test-orchestrated-resource-folder-${local.name_suffix}"
        os_policy_assignment_v1_payload {
            os_policies {
                id = "test-os-policy-folder-${local.name_suffix}"
                mode = "VALIDATION"
                resource_groups {
                    resources {
                        id = "resource-tf"
