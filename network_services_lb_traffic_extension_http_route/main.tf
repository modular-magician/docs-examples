...
resource "google_compute_network" "ilb_network" {
  name                    = "l7-ilb-network-route-${local.name_suffix}"
  auto_create_subnetworks = false
}

# proxy-only subnet
resource "google_compute_subnetwork" "proxy_subnet" {
  name          = "l7-ilb-proxy-subnet-route-${local.name_suffix}"
...
resource "google_compute_subnetwork" "ilb_subnet" {
  name          = "l7-ilb-subnet-route-${local.name_suffix}"
...
resource "google_compute_forwarding_rule" "default" {
  name                  = "l7-ilb-forwarding-rule-route-${local.name_suffix}"
  region                = "us-west1"
  ip_protocol           = "TCP"
...
resource "google_compute_region_target_http_proxy" "default" {
  name     = "l7-ilb-target-http-proxy-route-${local.name_suffix}"
  region   = "us-west1"
...
resource "google_compute_region_url_map" "default" {
  name            = "l7-ilb-regional-url-map-route-${local.name_suffix}"
  region          = "us-west1"
...
resource "google_compute_region_backend_service" "default" {
  name                  = "l7-ilb-backend-subnet-route-${local.name_suffix}"
  region                = "us-west1"
  protocol              = "HTTP"
  load_balancing_scheme = "INTERNAL_MANAGED"
  timeout_sec           = 10
...
resource "google_compute_instance_template" "instance_template" {
  name         = "l7-ilb-mig-template-route-${local.name_suffix}"
...
resource "google_compute_region_health_check" "default" {
  name     = "l7-ilb-hc-route-${local.name_suffix}"
  region   = "us-west1"
...
resource "google_compute_region_instance_group_manager" "mig" {
  name     = "l7-ilb-mig1-route-${local.name_suffix}"
  region   = "us-west1"
...
resource "google_compute_firewall" "fw_iap" {
  name          = "l7-ilb-fw-allow-iap-hc-route-${local.name_suffix}"
  direction     = "INGRESS"
  network       = google_compute_network.ilb_network.id
...
resource "google_compute_firewall" "fw_ilb_to_backends" {
  name          = "l7-ilb-fw-allow-ilb-to-backends-route-${local.name_suffix}"
  direction     = "INGRESS"
  network       = google_compute_network.ilb_network.id
...
resource "google_network_services_http_route" "default" {
  name                   = "l7-ilb-http-route-${local.name_suffix}"
  labels                 = {
    foo = "bar"
  }
...
resource "google_network_services_lb_traffic_extension" "default" {
  name     = "l7-ilb-traffic-ext-route-${local.name_suffix}"
...
resource "google_compute_instance" "vm_test" {
  name         = "l7-ilb-test-vm-route-${local.name_suffix}"
  zone         = "us-west1-b"
  machine_type = "e2-small"
...
resource "google_compute_instance" "callouts_instance" {
  name         = "l7-ilb-callouts-ins-route-${local.name_suffix}"
  zone         = "us-west1-a"
  machine_type = "e2-small"
...
resource "google_compute_instance_group" "callouts_instance_group" {
  name        = "l7-ilb-callouts-ins-group-route-${local.name_suffix}"
...
resource "google_compute_region_health_check" "callouts_health_check" {
  name     = "l7-ilb-callouts-hc-route-${local.name_suffix}"
  region   = "us-west1"
...
resource "google_compute_region_backend_service" "callouts_backend" {
  name                  = "l7-ilb-callouts-backend-route-${local.name_suffix}"
  region                = "us-west1"
  protocol              = "HTTP2"
  load_balancing_scheme = "INTERNAL_MANAGED"
  timeout_sec           = 10
