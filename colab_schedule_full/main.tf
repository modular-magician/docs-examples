resource "google_colab_runtime_template" "my_runtime_template" {
  name = "runtime-template-${local.name_suffix}"
...
resource "google_dataform_repository" "dataform_repository" {
  name = "dataform-repository-${local.name_suffix}"
...
resource "google_colab_schedule" "schedule" {
  display_name = "full-schedule-${local.name_suffix}"
  location = "REGION"
  allow_queueing = true
  ...
  end_time = "2014-10-10T15:01:23Z-${local.name_suffix}"
  ...
  create_notebook_execution_job_request {
    notebook_execution_job {
      ...
      dataform_repository_source {
        ...
        dataform_repository_resource_name = "projects/PROJECT_NAME/locations/REGION/repositories/${google_dataform_repository.dataform_repository.name}"
      }
